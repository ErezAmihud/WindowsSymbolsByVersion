...
FIXTURE = [
    ...
    (
        "general",
        0,
        "unpacked_dir/sources/setup.exe",
        "general.pdb",
...
def build(dest="."):
    ...
    _rm(boot_wim)
    subprocess.run(
        ["wimlib-imagex", "capture", boot_dir, boot_wim, "boot_1"], check=True, capture_output=True
    )
    ...
    for image in (1, 2):
        ...
        subprocess.run(
            ["wimlib-imagex", cmd, img_dir, install_wim, f"install_{image}"],
            check=True,
            capture_output=True,
        )
    ...
    with (
        open(os.path.join(dest, "manifest_general.out"), "w") as out,
...
if __name__ == "__main__":
    ...
    info = subprocess.run(
        ["wimlib-imagex", "info", "install.wim"], check=True, capture_output=True, text=True
    ).stdout
