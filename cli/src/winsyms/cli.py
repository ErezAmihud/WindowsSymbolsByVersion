...
def _add_index_args(parser):
    parser.add_argument("--index", metavar="FILE", help="use a local index.json instead of downloading (airgap)")
    parser.add_argument("--arch", help="only consider builds of this architecture (e.g. amd64, x86)")
...
def _print_entries(entries, file=sys.stdout):
    ...
    for e in sorted(entries, key=lambda e: (e["build"], e["arch"])):
        ...
        print(f"{e['build']:<16} {e['arch']:<6} {paths:<5} {e['uuid']:<36} {e['title']}", file=file)
...
def _resolve_one(args):
    ...
    if len(matches) > 1:
        ...
        raise SystemExit(f"error: {args.query!r} matches {len(matches)} builds; narrow with --arch, the full build number or the uuid")
    return matches[0]
...
def main(argv=None):
    ...
    p_get.add_argument("--scope", choices=["all", "system32"], default="all",
                       help="which binaries to cover (default: all)")
    p_get.add_argument("--path-prefix", metavar="P",
                       help="cover only binaries under this path inside the image (overrides --scope)")
    p_get.add_argument("--out", metavar="PATH",
                       help="symbol output directory (default: ./symbols); with --manifest-only, the manifest file path (default: ./manifest)")
    p_get.add_argument("--server", default=DEFAULT_SERVER, help=f"symbol server URL (default: {DEFAULT_SERVER})")
    p_get.add_argument("--manifest-only", action="store_true",
                       help="only write the manifest file, do not run pdblister (airgap: fetch here, download inside)")
    p_get.add_argument("--pdblister", metavar="PATH", help="pdblister binary (default: $WINSYMS_PDBLISTER or PATH lookup)")
